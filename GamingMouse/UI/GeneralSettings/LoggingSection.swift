        ...
        Section {
            Toggle(isOn: $detailedLoggingOn) {
                withDescription {
                    ...
                    Text(
                        "Enabling this option will log all input events, which may increase CPU usage while using \(GamingMouse.appName), but can be useful for troubleshooting."
                    )
                }
            }
    ...
    func exportLogs() {
        exportQueue.async {
            ...
            do {
                ...
                let predicate = NSPredicate(format: "subsystem == '\(GamingMouse.appBundleIdentifier)'")
                let entries = try logStore.getEntries(with: [],
                                                      at: position,
                                                      matching: predicate)
                let logs = entries
                    .compactMap { $0 as? OSLogEntryLog }
                    .filter { $0.subsystem == GamingMouse.appBundleIdentifier }
                    .suffix(100_000)
                ...
                let filePath = directory.appendingPathComponent("\(GamingMouse.appName).log")
