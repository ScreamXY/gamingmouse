                ...
                Section {
                    Toggle(isOn: $showInMenuBar.animation()) {
                        withDescription {
                            ...
                            if !showInMenuBar {
                                Text("To show the settings, launch \(GamingMouse.appName) again.")
                            }
                        }
                    }
...
extension GeneralSettings {
    enum URLs {
        static func withEnvironmentParametersAppended(for url: URL) -> URL {
            ...
            let gamingMouseVersion = "v\(GamingMouse.appVersion)"

            var components = URLComponents(url: url, resolvingAgainstBaseURL: false)!
            var queryItems = components.queryItems ?? []
            queryItems.append(contentsOf: [
                .init(name: "os", value: osVersion),
                .init(name: "gamingmouse", value: gamingMouseVersion)
            ])
